## TamedTable name
- No software product, GitHub repo, or npm package found with this exact name.
- Adjacent / unrelated hits:
  - Table Tamer (poker session manager — already noted below): https://automaticpoker.com/table-management-software/
  - TAME — Test Automation Made Easy (unrelated): https://www.tametest.com/
  - Tame (event/video platform, unrelated): https://www.capterra.com/p/180832/Tame/
  - `@thetrg/tamed` on npm (unrelated, not a table package): https://www.npmjs.com/package/@thetrg/tamed
- Pros: past-participle adjective reads unambiguously as "tables that have been brought under control" — no imperative-verb confusion; singular form fits SaaS naming patterns (Airtable, Smartsheet, DataFrame); inherits the "tame" intuition you already settled on.
- Cons: still uses the crowded "Table*" prefix; "tamed" can carry a faintly negative shade in other contexts ("a tamed version" = watered-down), though it doesn't bite in the messy-data domain.

## Alternative names

### Formerly TableTamer
- There is poker sw, but it seems abandoned: 
  https://www.reddit.com/r/poker/comments/piv3d4/table_tamer_dead/ 
  https://upswingpoker.com/poker-tools-software/table-tamer/
- There is a GutHub project for browser-based CSV visualization and transformation (0 stars):
  https://github.com/hexawulf/tabletamer 
  It uses:
  - TanStack Table (headless): 
    https://tanstack.com/table/latest
    https://github.com/tanstack/table
  - TanStack Query: https://tanstack.com/query/latest

### TameTables
- No software product or GitHub/npm package found with this exact name.
- "TAM" / "Applied TAM" hits in search are unrelated (insurance brokerage, applicant tracking):
  https://www1.appliedsystems.com/en-ca/solutions/for-brokers/brokerage-management-system/applied-tam/
- The phrase "tame messy spreadsheets" shows up as marketing copy on Topic Bolt's CSV tools page, but it isn't a product name:
  https://topicbolt.com/csv-tools/
- Pros: same "tame" metaphor as TableTamer (keeps the brand intuition), shorter (10 chars), alliterative, harder to misread as "TableTimer".
- Cons: reads as an imperative verb phrase ("tame tables") rather than a noun, which is unusual for a product name.

### TableWhisperer
- No software product or GitHub/npm package found with this exact name.
- Related "whisperer" projects in the data/document space (none are direct conflicts, but the pattern is crowded):
  - Data-Whisperer — NLP-to-Tableau viz builder: https://github.com/nmannheimer/Data-Whisperer
  - LLMWhisperer — document/table extraction for LLMs: https://unstract.com/llmwhisperer/ and https://github.com/Zipstack/llmwhisperer-table-extraction
  - port-whisperer — CLI listing local ports: https://github.com/LarsenCundric/port-whisperer
- Pros: evokes a clear personality (patient, skilled handler of messy data), distinct from TableTimer, noun-shaped.
- Cons: "X-whisperer" is a tired naming trope at this point; longer (15 chars); sits in a category where several adjacent "whisperer" tools already exist.

### SheetShepherd
- No software product, GitHub repo, or npm package found with this exact name.
- Adjacent / unrelated hits (the "Shepherd" namespace is crowded):
  - Shepherd JS — well-known site-tour library, biggest collision risk for developer mindshare: https://shepherdjs.dev/ and https://github.com/shepherd-pro/react-shepherd
  - Shepherd Veterinary Software: https://www.shepherd.vet/
  - NerdWallet's Shepherd — repo-migration CLI: https://github.com/NerdWalletOSS/shepherd
  - GhostManager/Shepherd — red-team domain manager: https://github.com/GhostManager/Shepherd
- Pros: strong personality (patient, guiding, gentle); alliterative; "sheet" maps cleanly to spreadsheet/CSV mental model; uncrowded as an exact phrase.
- Cons: developers may associate "Shepherd" first with the site-tour library; longer (14 chars); "shepherd" implies guiding/herding which is a slight metaphor stretch for a transform tool.

### RowRanger
- No software product, GitHub repo, or npm package found with this exact name.
- Adjacent hits — one notable semantic collision:
  - **Apache Ranger** — does "row-level filtering and column-masking" for Hive/big-data, which is uncomfortably close to what "RowRanger" suggests semantically. Worth flagging as a possible mental-model collision for data engineers: https://cwiki.apache.org/confluence/display/RANGER/Row-level+filtering+and+column-masking+using+Apache+Hive+policies+in+Apache+Hive
  - Rank Ranger — SEO tool, unrelated: https://www.warebuy.com/product/rank-ranger-software/
  - `ranger` npm package and various `*-ranger` packages exist: https://www.npmjs.com/package/ranger, https://www.npmjs.com/package/react-ranger
- Pros: punchy, alliterative, Western-frontier personality; short (9 chars); evokes scanning/patrolling rows which fits CSV cleanup well.
- Cons: Apache Ranger overlap means data-eng audiences may mis-categorize the product as a security/access-control tool; "row" is narrower than "table" or "sheet" — implies row-level work and may undersell column/structural transforms.